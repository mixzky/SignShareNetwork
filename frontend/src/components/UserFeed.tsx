...
export default function UserFeed() {
  ...
  useEffect(() => {
    ...
    async function fetchUserAndActivities() {
      makeAnnouncement("Loading activity feed...");
      ...
      if (user) {
        ...
        if (!error && activitiesData && activitiesData.length > 0) {
          ...
          makeAnnouncement(`Loaded ${activitiesWithTitles.length} activities`);
        } else {
          setActivities([]);
          makeAnnouncement("No recent activities found");
        }
      }
      setLoading(false);
    }
    ...
    <>
      ...
      >
        ...
        <div
          ...
          aria-describedby="activity-count"
        >
          <div id="activity-count" className="sr-only">
            {activities.length === 0
              ? "No activities"
          ...
          {activities.map((activity) => {
            ...
            const getActivityDescription = (activity: Activity) => {
              ...
              if (activity.video_title) {
                description += ` for video: ${activity.video_title}`;
              }
            ...
            return (
              <div
                ...
                role="listitem"
                aria-label={getActivityDescription(activity)}
                tabIndex={0}
              >
                {/* Icon based on activity type */}
                <span aria-label={`${activity.type} activity`} role="img">
