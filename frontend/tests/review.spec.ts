...
test.describe('Video Review System', () => {
  ...
  test.describe('Review Form', () => {
    test("review form is accessible", async ({ page }) => {
      ...
      await expect(page.getByRole("textbox", { name: /write a review/i })).toBeVisible();
    ...
    test("successful review submission", async ({ page }) => {
      ...
      // Fill review form
      await page.getByRole("textbox", { name: /write a review/i }).fill("This is a great video!");
    ...
    test("review form validation", async ({ page }) => {
      ...
      // Review too long
      await page.getByRole("textbox", { name: /write a review/i }).fill("a".repeat(1001));
      await page.getByRole("button", { name: /submit/i }).click();
  ...
  test.describe('Review Display', () => {
    test("reviews are displayed correctly", async ({ page }) => {
      ...
      await page.getByRole("textbox", { name: /write a review/i }).fill("Test review content");
    ...
    test("review sorting", async ({ page }) => {
      ...
      // Verify sort options
      await page.getByRole("combobox", { name: /sort reviews/i }).selectOption("newest");
      await expect(page.url()).toContain("sort=newest");
      
      await page.getByRole("combobox", { name: /sort reviews/i }).selectOption("highest-rated");
      await expect(page.url()).toContain("sort=highest-rated");
      
      await page.getByRole("combobox", { name: /sort reviews/i }).selectOption("lowest-rated");
    ...
    test("review pagination", async ({ page }) => {
      ...
      await expect(page.getByRole("navigation", { name: /review pagination/i })).toBeVisible();
  ...
  test.describe('Review Management', () => {
    test("user can edit their review", async ({ page }) => {
      ...
      await page.getByRole("textbox", { name: /write a review/i }).fill("Initial review");
      await page.getByRole("radio", { name: /3 stars/i }).click();
      await page.getByRole("button", { name: /submit/i }).click();
      
      // Edit review
      await page.getByRole("button", { name: /edit review/i }).click();
      await page.getByRole("textbox", { name: /edit review/i }).fill("Updated review");
      await page.getByRole("radio", { name: /4 stars/i }).click();
    ...
    test("user can delete their review", async ({ page }) => {
      ...
      // Submit review
      await page.getByRole("textbox", { name: /write a review/i }).fill("Review to delete");
      await page.getByRole("radio", { name: /3 stars/i }).click();
      await page.getByRole("button", { name: /submit/i }).click();
    ...
    test("user cannot edit other users' reviews", async ({ page }) => {
      ...
      const otherUserReview = page.getByTestId("review-item").filter({ hasText: "other_user" });
      await expect(otherUserReview.getByRole("button", { name: /edit/i })).not.toBeVisible();
      await expect(otherUserReview.getByRole("button", { name: /delete/i })).not.toBeVisible();
    });
  });
  ...
  test.describe('Review Interactions', () => {
    ...
    test("user can report inappropriate reviews", async ({ page }) => {
      ...
      await page.getByRole("textbox", { name: /additional details/i }).fill("Test report reason");
  ...
  test.describe('Error Handling', () => {
    test("network error during review submission", async ({ page }) => {
      ...
      await page.getByRole("textbox", { name: /write a review/i }).fill("Test review");
      await page.getByRole("radio", { name: /5 stars/i }).click();
      await page.getByRole("button", { name: /submit/i }).click();
    ...
    test("retry functionality", async ({ page }) => {
      ...
      await page.route('**/api/reviews**', route => route.abort('failed'), { times: 1 });
